...
def upgrade() -> None:
	...
	op.create_table(
		...
		sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("NOW()")),
		sa.UniqueConstraint("user_id", "provider_msg_id", name="uq_messages_user_provider"),
	)
	...
	op.create_table(
		...
		sa.Column("geom", sa.String().with_variant(Geography(geometry_type="LINESTRING", srid=4326), "postgresql"), nullable=True),
		sa.Column("meta_json", sa.JSON, nullable=True),
		sa.UniqueConstraint("trip_id", "airline", "flight_number", "dep_time", name="uq_segments_trip_flight"),
	)

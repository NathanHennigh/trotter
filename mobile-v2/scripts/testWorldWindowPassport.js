...
assert(passportPageDescription({kind:'record',stamps:[]}, {...identity,register:[{label:'Most common route',value:'DFW → IAH',detail:'4 flights'}]}).lines.includes('Most common route: DFW → IAH, 4 flights'), 'The typeset travel register exposes its real fields');
...
const { paperGestureIntent, selectedActivityIndex } = component('passport-gestures.ts');
assert.equal(paperGestureIntent(3, 2), 'pending');
assert.equal(paperGestureIntent(2, 35), 'scroll', 'Vertical movement remains a page scroll');
assert.equal(paperGestureIntent(-35, 8), 'turn', 'Horizontal intent can grab anywhere');
assert.equal(paperGestureIntent(16, 15), 'turn', 'Natural diagonal page grabs remain possible');
assert.equal(selectedActivityIndex([{year:2016},{year:2020},{year:2025}], 2020), 1);
assert.equal(selectedActivityIndex([{year:2014},{year:2016},{year:2020},{year:2025}], 2020), 2, 'Importing earlier history preserves the actual chosen year');
const { earnedCountries } = component('passport-earned.ts');
assert.deepEqual(earnedCountries(null, {codes:['SG'],syncedAt:'2026-09-14'}), [], 'Initial hydration does not celebrate existing countries');
assert.deepEqual(earnedCountries({codes:['SG'],syncedAt:'one'}, {codes:['SG','NL'],syncedAt:'one'}), [], 'A rerender or filter never simulates a completed import');
assert.deepEqual(earnedCountries({codes:['SG'],syncedAt:'one'}, {codes:['SG','NL'],syncedAt:'two'}), ['NL']);
assert.deepEqual(earnedCountries({codes:['SG','NL'],syncedAt:'two'}, {codes:['SG','NL'],syncedAt:'two'}), [], 'An earned stamp never replays on the same import');
const { passportScope, scopedPassportArchive } = component('passport-scope.ts');
const scope2024 = passportScope([connected], '2024');
assert.equal(scope2024.trips[0].flightCount, 1);
assert.deepEqual(scope2024.arrivals.map(a=>a.country), ['Netherlands'], 'Scoped destination metadata cannot invent a Singapore arrival in another year');
assert.equal(component('passport-collection-model.ts').tripsForCountry(scope2024.trips, arrivals.find(a=>a.country==='Singapore'), true).length, 0, 'Scoped country journeys require an actual in-year arrival, not the trip primary destination');
const scopeArchive = scopedPassportArchive([connected], profile, '2025');
assert.equal(scopeArchive.flights, 2); assert.equal(scopeArchive.miles, 2000);
assert.equal(scopeArchive.firstFlightDate, '2024-12-31', 'Year scope retains lifetime traveler identity');
assert.equal(scopeArchive.recordStartDate, '2025-01-01', 'The scoped travel register starts with that year, separately from lifetime identity');
assert.equal(scopeArchive.arrivals.find(a=>a.country==='United Arab Emirates').firstVisitDate, arrivals.find(a=>a.country==='United Arab Emirates').firstVisitDate, 'Filtering does not rewrite first-entry date or airport');
assert.equal(JSON.stringify(connected), before, 'Scoped metrics never mutate the underlying trip');

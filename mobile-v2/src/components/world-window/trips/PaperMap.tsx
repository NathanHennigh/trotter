...
var geoAttribution='<a href="https://www.geoapify.com/" target="_blank" rel="noopener">Geoapify</a>',hasGeoAttribution=false;
...
window.drawTravelMap=function(next){data=next;var usesGeo=data.points.some(function(p){return p.provider==='geoapify'});if(usesGeo&&!hasGeoAttribution)map.attributionControl.addAttribution(geoAttribution);if(!usesGeo&&hasGeoAttribution)map.attributionControl.removeAttribution(geoAttribution);hasGeoAttribution=usesGeo;var origin=anchor(data.points);data.points.forEach(function(p){p.displayLon=(p.lon+360)%360;if(p.displayLon<origin)p.displayLon+=360});pins.clearLayers();paths.clearLayers();var byId={};data.points.forEach(function(p){byId[p.id]=p});(data.lines||[]).forEach(function(line){var a=byId[line.from],b=byId[line.to];if(a&&b)L.polyline([[a.lat,a.displayLon],[b.lat,b.displayLon]],{color:'#a1663d',weight:2,opacity:.85}).addTo(paths)});
        ...
        <WebView
          ...
          onShouldStartLoadWithRequest={(request) => {
            if (
              ...
              return true;
            if (["https://www.openstreetmap.org/copyright", "https://www.geoapify.com/"].includes(request.url))

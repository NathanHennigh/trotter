...
export function placeSymbol(category: string): PlaceSymbol {
  ...
  if (/\b(bookshops?|bookstores?|books?|libraries|library)\b/.test(value))
    return "bookshop";
  if (
    /\b(lookouts?|viewpoints?|observation|miradouro|binoculars?)\b/.test(value)
  )
    return "lookout";
  if (
    /\b(cafes?|cafeterias?|coffee|tearooms?|tea room|bakeries|bakery|patisseries?)\b/.test(
      value,
    )
  )
    return "cafe";
  if (
    /\b(restaurants?|dining|eateries|eatery|bistros?|brasseries?)\b/.test(value)
  )
    return "restaurant";
  if (
    /\b(hotels?|hostels?|lodging|stays?|resorts?|inns?|guesthouses?|guest house)\b/.test(
      value,
    )
  )
    return "hotel";
  if (
    /\b(walks?|walking|trails?|hikes?|hiking|paths?|promenades?)\b/.test(value)
  )
    return "walk";
  if (
    /\b(landmarks?|monuments?|museums?|architecture|historic|temples?|palaces?|attractions?)\b/.test(
      value,
    )
  )
    return "landmark";

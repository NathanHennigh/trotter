...
import { WWIcon, WWEmpty, WWButton } from "../WorldWindowUI";
...
export function CollectionButtons({ archive, onOpen }: { archive: PassportArchive; onOpen: (kind: CollectionKind) => void }) {
  ...
  return <View style={styles.collections}>{(["countries", "airports", "airlines"] as const).map((kind, i) => <Pressable key={kind} onPress={() => onOpen(kind)} accessibilityRole="button" accessibilityLabel={`${values[kind]} ${kind}`} style={({ pressed }) => [styles.collection, pressed && styles.pressed]}>
    <View style={styles.mark} accessible={false}><View style={{ width: kind === "airports" ? 60 : 72, height: 72, overflow: "hidden" }}><Image source={emblems} style={{ position: "absolute", width: 201.6, height: 134.4, left: -64.8 * i - (kind === "airports" ? 6 : 0), top: -31.2 }} /></View></View>
...
export function CollectionBack({ label, onBack }: { label: string; onBack: () => void }) {
  return <Pressable style={styles.back} accessibilityRole="button" accessibilityLabel={`Back to ${label}`} onPress={onBack}><WWIcon name="back" size={18} /><Text style={styles.backLabel}>{label}</Text></Pressable>;
}
export function CollectionTitle({ title, count, onBack, backLabel = "Passport" }: { title: string; count?: number; onBack: () => void; backLabel?: string }) {
  const { width, fontScale } = useWindowDimensions();
  const largeText = fontScale >= 1.35;
  const fitTitle = largeText ? { fontSize: Math.min(32, (getMobileVisualWidth(width) - 48) / (fontScale * Math.max(4.8, ...title.split(/\s+/).map(word => word.length * .56)))) } : undefined;
  return <View style={[styles.titleBar, largeText && styles.stacked]}><Pressable style={styles.titleBack} accessibilityRole="button" accessibilityLabel={`Back to ${backLabel}`} onPress={onBack}><WWIcon name="back" size={21} /></Pressable><View style={[styles.collectionTitleRow, largeText && styles.titleRowLarge]}><Text accessibilityRole="header" style={[styles.heading, fitTitle]}>{title}</Text>{count !== undefined && <Text style={styles.headingCount}>{count}</Text>}</View></View>;
}
...
export function CountryIndex({ arrivals, onSelect, query = "" }: { arrivals: CountryArrival[]; onSelect: (arrival: CountryArrival) => void; query?: string }) {
  const { width, fontScale } = useWindowDimensions(), columnWidth = fontScale >= 1.35 ? getMobileVisualWidth(width) - 48 : (getMobileVisualWidth(width) - 48 - 18) / 2;
...
export function CountryArrivalDetail({ arrival, trips, onBack, onOpenTrip, width, backLabel = "Countries", onBackHandlerChange }: { arrival: CountryArrival; trips: TripSummary[]; onBack: () => void; onOpenTrip?: (trip: TripSummary) => void; width: number; backLabel?: string; onBackHandlerChange?: (handler: (() => boolean) | null) => void }) {
  ...
  const relevant = React.useMemo(() => tripsForCountry(trips, arrival), [trips, arrival]);
  const city = relevant.flatMap(t => t.segments ?? []).find(s => s.arrAirport === arrival.airportCode)?.arrPoint?.city;
  return <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}><View style={styles.inset}>
    <CollectionTitle title={showTrips ? `Trips to ${arrival.country}` : arrival.country} backLabel={showTrips ? arrival.country : backLabel} onBack={showTrips ? () => setShowTrips(false) : onBack} />
...
export function CollectionList({ kind, archive, onBack, onSelectCountry, onOpenTrip, backLabel, onBackHandlerChange }: { kind: CollectionKind; archive: PassportArchive; onBack: () => void; onSelectCountry: (arrival: CountryArrival) => void; onOpenTrip?: (trip: TripSummary) => void; backLabel?: string; onBackHandlerChange?: (handler: (() => boolean) | null) => void }) {
  ...
  if (detail) {
    ...
    return <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}><View style={styles.inset}><CollectionTitle title={kind === "airports" ? "Airport" : "Airline"} backLabel={kind === "airports" ? "Airports" : "Airlines"} onBack={() => setDetail(null)}/>
      {"city" in detail ? <><View style={[styles.airportPortrait, largeText && styles.stacked]}><View style={{ flex: 1 }}><Text style={styles.airportCode} numberOfLines={1} adjustsFontSizeToFit>{detail.code}</Text><Text style={styles.airportCity}>{detail.city || detail.code}</Text>{detail.country ? <Text style={styles.meta}>{detail.country}</Text> : null}</View><View style={[styles.countCopy, largeText && styles.portraitCountInline]}><Text style={styles.portraitCount}>{detail.flights}</Text><Text style={styles.meta}>flights</Text></View></View>
        {dates.length > 0 && <View style={[styles.visitDates, largeText && styles.stacked]}><View style={styles.dateCopy}><Text style={styles.meta}>First recorded visit</Text><Text style={styles.visitDate}>{readableDate(dates[0])}</Text></View><View style={styles.dateCopy}><Text style={styles.meta}>Latest recorded visit</Text><Text style={styles.visitDate}>{readableDate(dates.at(-1))}</Text></View></View>}
  ...
  return <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
    ...
    {kind === "countries" ? <CountryIndex arrivals={archive.arrivals} onSelect={onSelectCountry} query={query}/> : <View style={styles.inset}>{filtered.map(item => <Pressable key={item.code} accessibilityRole="button" onPress={() => setDetail(item)} style={({ pressed }) => [styles.listRow, pressed && styles.pressed]}>
      {kind === "airlines" ? <AirlineLogo code={item.code} size={40}/> : <Text style={[styles.airportTag, largeText && styles.airportTagLarge]} numberOfLines={1}>{item.code}</Text>}
      <View style={styles.listCopy}><Text style={styles.airlineName}>{kind === "airlines" ? airlineName(item.code) : "city" in item ? item.city || item.code : item.code}</Text><Text style={styles.meta}>{kind === "airlines" ? item.code : "country" in item ? item.country : ""}</Text>{largeText && <Text style={styles.meta}>{item.flights} {item.flights === 1 ? "flight" : "flights"}</Text>}</View>
      {!largeText && <View style={styles.countCopy}><Text style={styles.flightCount}>{item.flights}</Text><Text style={styles.meta}>{item.flights === 1 ? "flight" : "flights"}</Text></View>}<WWIcon name="arrow" size={16}/>
...
const styles = StyleSheet.create({
  ...
  collectionTitle: { fontFamily: fonts.sans, fontSize: 13, color: colors.ink, textAlign: "center", alignSelf: "stretch" },
  pressed: { opacity: .68 },
  ...
  titleBar: { flexDirection: "row", alignItems: "flex-start", gap: 8, paddingTop: 18, paddingBottom: 18, borderBottomWidth: 1, borderBottomColor: colors.paperBorder },
  titleBack: { minWidth: 44, minHeight: 44, alignItems: "flex-start", justifyContent: "center" },
  collectionTitleRow: { flex: 1, flexDirection: "row", flexWrap: "wrap", alignItems: "baseline", columnGap: 14, rowGap: 4, paddingTop: 5 },
  titleRowLarge: { width: "100%", flex: 0 },
  heading: { flexShrink: 1, fontFamily: fonts.display, fontSize: 32, lineHeight: 36, letterSpacing: -.5, color: colors.ink },
  ...
  countryGrid: { paddingHorizontal: 24, flexDirection: "row", flexWrap: "wrap", rowGap: 24, columnGap: 18 },
  ...
  portraitCountInline: { flexDirection: "row", alignItems: "baseline", gap: 10 },
